...
def battledraw():
    ...
    textHp = text.fonthp.render(str(sta.mc.Hp) + '/' + str(sta.mc.maxHp), True, black)
    if sta.partyamount >=2:
      textHp1 = text.fonthp.render(str(sta.pm1.Hp) + '/' + str(sta.pm1.maxHp), True, black)
    if sta.partyamount ==3:
      textHp2 = text.fonthp.render(str(sta.pm2.Hp) + '/' + str(sta.pm2.maxHp), True, black)
    textXp = text.fonthp.render(str(Xp.gxp) + '/' + str(round(Xp.xpn)), True, black)
    if sta.partyamount >=2:
      textXp1 = text.fonthp.render(str(Xp.pm1gxp) + '/' + str(round(Xp.pm1xpn)), True, black)
    if sta.partyamount ==3:
      textXp2 = text.fonthp.render(str(Xp.pm2gxp) + '/' + str(round(Xp.pm2xpn)), True, black)
    ...
    if not attacktest:
      ...
      playerhpbar = p.draw.rect(s.ds, red, p.Rect(70, 230, 100, 15)), p.draw.rect(s.ds, green, p.Rect(70, 230, playerhp, 15))
      s.ds.blit(textHp, text.textRectHp)
      playerxpbar = p.draw.rect(s.ds, (0, 101, 255),p.Rect(180, 230, 100, 15)),p.draw.rect(s.ds, (135, 206, 250),p.Rect(180, 230, 100 - (playerxp +1),15))
      playerRectlv = text.playerlv.get_rect()
      playerRectlv.topleft = (290,230)
      ...
      if not sta.pm1.Hp <= 0 and sta.partyamount >= 2:
        ...
        pm1xpbar = p.draw.rect(s.ds, (0, 101, 255),p.Rect(180, 250, 100, 15)),p.draw.rect(s.ds, (135, 206, 250),p.Rect(180, 250, 100 - (pm1xp +1),15))
        pm1Rectlv = text.pm1lv.get_rect()
        pm1Rectlv.topleft = (290,250)
      ...
      if not sta.pm2.Hp <= 0 and sta.partyamount == 3:
        ...
        pm2xpbar = p.draw.rect(s.ds, (0, 101, 255),p.Rect(180, 270, 100, 15)),p.draw.rect(s.ds, (135, 206, 250),p.Rect(180, 270, 100 - (pm2xp +1),15))
        pm2Rectlv = text.pm2lv.get_rect()
        pm2Rectlv.topleft = (290,270)
...
attacktest=False
def attackcheck():
  global attacktest,selectorover,testing
